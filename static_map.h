...
size_t HM_CDECL hm_db_place_size(unsigned int elements);
...
hm_error_t HM_CDECL hm_compile(char *db_place, size_t db_place_size,
...
size_t HM_CDECL hm_place_used(const hm_database_t *db);

// hm_find returns the value corresponding to the given IP in the database.
uint64_t HM_CDECL hm_find(const hm_database_t *db, const uint32_t ip);
...
// during the call.
hm_error_t HM_CDECL hm_db_from_place(char *db_place, size_t db_place_size,
                                     hm_database_t **db);
